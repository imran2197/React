...
function NavBar() {
  ...
  const [selectedNav, setSelectedNav] = useState("Movies");
  ...
  return (
    <nav className="relative bg-black text-white">
      <div className="flex items-center justify-between">
        <Link
          to="/"
          className="flex items-center gap-3 text-2xl font-bold yellowColor px-6 py-4"
        >
      ...
      <div className="hidden md:block absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2">
        <ul className="relative flex gap-8 text-xl font-medium">
          {navData.map((data) => (
            <li
              className={`${selectedNav == data.label ? "active" : ""}`}
              key={data.label}
              onClick={() => setSelectedNav(data.label)}
            >
              <Link to={data.link} className="px-2 py-2">
                {data.label}
              </Link>
            </li>
          ))}
      ...
      {open && (
        <ul className="md:hidden mt-4 flex flex-col gap-2 bg-[#111]">
          {navData.map((data) => (
            <li key={data.label} onClick={() => setSelectedNav(data.label)}>
              <Link
                to={data.link}
                onClick={() => setOpen(false)}
                className={`block px-4 py-3 ${
                  selectedNav === data.label
                    ? "yellowColor border-l-4 border-[#ffa300] bg-yellow-500/10"
                    : ""
                }`}
              >
                {data.label}
